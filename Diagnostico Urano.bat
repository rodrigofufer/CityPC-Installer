...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; try{ (New-Object Net.WebClient).DownloadFile('%GITHUB_RAW%/%VERSION_FILE%','%temp%\citypc_ver_diag_ur.txt') }catch{}" >nul 2>&1

if not exist "%temp%\citypc_ver_diag_ur.txt" (
...
set "REMOTE_VER="
set /p REMOTE_VER=<"%temp%\citypc_ver_diag_ur.txt"
del /F /Q "%temp%\citypc_ver_diag_ur.txt" >nul 2>&1
